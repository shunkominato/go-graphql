...
func (uc *UserCreate) check() error {
	if _, ok := uc.mutation.Provider(); !ok {
		return &ValidationError{Name: "provider", err: errors.New(`model: missing required field "User.provider"`)}
	}
	if _, ok := uc.mutation.UID(); !ok {
		return &ValidationError{Name: "uid", err: errors.New(`model: missing required field "User.uid"`)}
	}
	if _, ok := uc.mutation.EncryptedPassword(); !ok {
		return &ValidationError{Name: "encrypted_password", err: errors.New(`model: missing required field "User.encrypted_password"`)}
	}
