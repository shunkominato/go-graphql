...
func (tc *TodoCreate) check() error {
	if _, ok := tc.mutation.Todo(); !ok {
		return &ValidationError{Name: "todo", err: errors.New(`model: missing required field "Todo.todo"`)}
	}
	if _, ok := tc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`model: missing required field "Todo.created_at"`)}
	}
	if _, ok := tc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`model: missing required field "Todo.updated_at"`)}
	}
	return nil
}
