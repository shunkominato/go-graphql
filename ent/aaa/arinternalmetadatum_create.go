...
func (aimc *ArInternalMetadatumCreate) check() error {
	if _, ok := aimc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`aaa: missing required field "ArInternalMetadatum.created_at"`)}
	}
	if _, ok := aimc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`aaa: missing required field "ArInternalMetadatum.updated_at"`)}
	}
	return nil
}
