...
func (tq *TodoQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (tq *TodoQuery) FirstIDX(ctx context.Context) uint {
...
func (tq *TodoQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (tq *TodoQuery) OnlyIDX(ctx context.Context) uint {
...
func (tq *TodoQuery) IDs(ctx context.Context) (ids []uint, err error) {
...
func (tq *TodoQuery) IDsX(ctx context.Context) []uint {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *TodoQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("aaa: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (tq *TodoQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range tq.inters {
		if inter == nil {
			return fmt.Errorf("aaa: uninitialized interceptor (forgotten import aaa/runtime?)")
		}
	...
	for _, f := range tq.ctx.Fields {
		if !todo.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("aaa: invalid field %q for query", f)}
		}
	}
	if tq.path != nil {
...
func (tq *TodoQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(todo.Table, todo.Columns, sqlgraph.NewFieldSpec(todo.FieldID, field.TypeUint))
	_spec.From = tq.sql
